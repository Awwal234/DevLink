...
function logout() {
  ...
  router.push("/");
  hanko!.user.logout();
  localStorage.setItem("hanko_session", null);
  localStorage.setItem("hanko", null);
  localStorage.setItem("hanko_login", false);
}
function showWarning() {
  const hanko_logged = localStorage.getItem("hanko_login");
  if (hanko_logged == "true") {
    warning.value = true;
    setTimeout(() => {
      warning.value = false;
    }, 8000);
...
<template>
  <div
    class="md:py-[40px] overflow-x-hidden bg-[#fff] rounded-[12px] py-[20px]"
  >
    <div class="px-[24px] md:px-[40px]">
      <div
        class="text-[#333] mb-[8px] text-[24px] md:text-[32px] font-[700] leading-[150%]"
      >
        Customize your links
      </div>
      <div
        class="text-[#737373] mb-[40px] text-[16px] font-[400] leading-[150%]"
      >
        Add/edit/remove links below and then share all your profiles with the
        world!
      </div>
      <!--if user log in with hanko :))-->
      <div
        v-show="warning"
        class="bg-[#fff] mb-[30px] shadow-sm py-[6px] px-[8px] font-[400] text-[14px] md:text-[16px] rounded-[8px] text-[#000]"
      >
        User you logged in with hanko auth :)) Please note you won&apos;t be
        able to create a link and edit.
      </div>
      <!---->
      <button
        @click="validateInput"
      ...
      >
        ...
        <div
          class="text-center leading-[150%] text-[#737373] text-[16px] font-[400]"
        >
      ...
      <!--links-->
      <div
        class="mb-[24px] overflow-x-hidden rounded-[12px] p-[20px] w-full addlink_container"
      >
        <div class="text-[12px] mb-[4px] font-[400] text-[#333] leading-[150%]">
          Platform
        </div>
        <div
          :class="drop ? 'border-[#633cff] inp_shadowww' : 'border-[#d9d9d9]'"
        ...
        >
          ...
          <div class="w-[10%] items-right">
            <img
              @click="drop = !drop"
              src="/img/chevron.svg"
              class="w-fit float-right cursor-pointer"
            />
          </div>
        </div>
        <!--dropdown links-->
        <div v-show="drop" class="w-fit">
          ...
          >
            ...
            >
              ...
              <div
                class="border-b-[1px] mb-[12px] w-full border-[#d9d9d9]"
              ></div>
            </div>
          </div>
        </div>
        ...
        <div
          :class="inp2 ? 'border-[#633cff] inp_shadowww' : 'border-[#d9d9d9]'"
          class="py-[12px] mb-[12px] flex items-center border-[1px] px-[16px] rounded-[8px] bg-[#fff]"
        >
      ...
      <div class="h-[300px] side md:h-[300px] overflow-scroll">
        <div
          v-for="links in linksset"
          :key="links.id"
          class="mb-[24px] overflow-x-hidden rounded-[12px] p-[20px] w-full addlink_container"
        >
          <div
            class="text-[12px] mb-[4px] font-[400] text-[#333] leading-[150%]"
          >
            Platform
          </div>
          <div
          ...
          >
            <div class="w-[5%] items-right">
              <img
                src="/img/link.svg"
                class="w-fit float-left cursor-pointer"
              />
            </div>
            <div class="w-[95%]">
              <input
    ...
    <div
      class="w-full mb-[16px] md:mb-[24px] border-b-[1px] border-[#d9d9d9]"
    ></div>
